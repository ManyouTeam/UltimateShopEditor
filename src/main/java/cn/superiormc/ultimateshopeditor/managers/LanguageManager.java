...
public class LanguageManager {
    ...
    private void initLanguage() {
        this.file = new File(UltimateShopEditor.instance.getDataFolder(), "message.yml");
        if (!file.exists()){
            Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[UltimateShopEditor] §cWe can not found your message file, " +
                    "please try restart your server!");
        }
        else {
            this.messageFile = YamlConfiguration.loadConfiguration(file);
        }
        InputStream is = UltimateShopEditor.instance.getResource("message.yml");
        if (is == null) {
            return;
        }
        this.tempFile = new File(UltimateShopEditor.instance.getDataFolder(), "tempMessage.yml");
        try {
    ...
    public void sendStringText(String... args) {
        ...
        if (text == null) {
            if (this.tempMessageFile.getString(args[0]) == null) {
                Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[UltimateShopEditor] §cCan not found language key: " + args[0] + "!");
                return;
            }
            else {
                Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[UltimateShopEditor] §cUpdated your language file, added " +
    ...
    public void sendStringText(Player player, String... args) {
        ...
        if (text == null) {
            if (this.tempMessageFile.getString(args[0]) == null) {
                player.sendMessage("§x§9§8§F§B§9§8[UltimateShopEditor] §cCan not found language key: " + args[0] + "!");
                return;
            }
            else {
                Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[UltimateShopEditor] §cUpdated your language file, added " +
